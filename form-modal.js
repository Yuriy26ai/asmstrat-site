...
(function () {
  ...
  /* ---------- Кнопка в секции контактов ----------
     Контакты (email/телефон/telegram) теперь живут в подвале, поэтому
     в секции #contacts оставляем только призыв + кнопку заявки.
     Привязываемся к контейнеру секции; если старый список .clinks ещё
     присутствует — вставляем перед ним (обратная совместимость). */
  var contactBox = document.querySelector('.contact .wrap');
  if (contactBox) {
    var clinks = contactBox.querySelector('.clinks');
    ...
    if (clinks) {
      ...
      inlineBtn.style.marginBottom = '0';
      contactBox.appendChild(inlineBtn);
    }
  }
